...
namespace plcdemo
{
    public partial class PL : Form
    {
            ...
            if (IntPtr.Zero != h)
            {
                ...
                if (ret >= 0)
                {
                    ...
                    for (int k = 0; k < lv_sel_count; k++)
                    {
                        ...
                        if (sub_str.Length >= 2)
                        {
                            MessageBox.Show(this.lsvselparam.Items[k].Text);
        ...
        private void btnParseDData_Click_1(object sender, EventArgs e)
        {
            ...
            txtSDData.AppendText("卡号     工号     验证方式     门ID     事件   出入状态     时间");
            ...
            return;
            //if (fsFile.Read(FileBuf, 0, (int)fsFile.Length) != (int)fsFile.Length)
            {
                //     MessageBox.Show("读文件出错！");
            }
            /*
            while (BufferSize > 0)
            {
                AcessLog = (TC3AcessLog)RawDeserialize(FileBuf, pos, typeof(TC3AcessLog));

                LogLen = Marshal.SizeOf(AcessLog);

                LogStr = AcessLog.cardno.ToString() + "  " + AcessLog.pin.ToString() + "  " + AcessLog.verified.ToString() + "  " + AcessLog.doorID.ToString() + "  " + AcessLog.EventType.ToString()
                        + "  " + AcessLog.inoutstate.ToString() + "  " + AcessLog.time_second.ToString();                      

               // lvAccessLog.Items.Add(LogStr);
                TexAccessLog.Text += LogStr + "\n";
                BufferSize -= LogLen;
                pos += LogLen;
            }
