...
export class NguiAutoCompleteDirective implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formControl.valueChanges.subscribe(value => {
      if (value) {
        this.selectNewValue(value, false);
      }
    });
    // apply toString() method for the object
    if (!!this.ngModel) {
      this.selectNewValue(this.ngModel);
    } else if (!!this.formControl && this.formControl.value && this.displayPropertyName) {
      this.selectNewValue(this.formControl.value[this.displayPropertyName], false);
  ...
  selectNewValue = (item: any, emitEvent = true) => {
    ...
    // make return value
    let val = item;
    if (this.selectValueOf && item && item[this.selectValueOf]) {
      val = item[this.selectValueOf];
    }
    if (emitEvent) {
      if ((this.parentForm && this.formControlName) || this.extFormControl) {
        if (!!val) {
          this.formControl.patchValue(val, {emitEvent});
        }
      }
